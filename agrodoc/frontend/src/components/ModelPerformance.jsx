...
// Shorten "Disease name" from "Crop - Disease name" for Y-axis labels
const shortLabel = (cls) => {
  const parts = cls.split(' - ')
  if (parts.length < 2) return cls.length > 22 ? cls.slice(0, 20) + '…' : cls
  const [crop, ...rest] = parts
  const disease = rest.join(' - ')
  if (disease.toLowerCase() === 'healthy') return `${crop} · healthy`
  // Abbreviate a few well-known long names
  const abbrev = disease
    .replace('Haunglongbing (Citrus greening)', 'HLB / Citrus greening')
  ...
  return abbrev.length > 22 ? abbrev.slice(0, 20) + '…' : abbrev
}
...
// Custom tooltip for the per-class bar charts
const BarTip = ({ active, payload }) => {
  if (!active || !payload?.length) return null
  const d = payload[0].payload
  return (
    <div style={{ ...TIP_STYLE.contentStyle, padding: '8px 12px', minWidth: 200 }}>
      <p style={{ fontWeight: 700, color: '#2c1a0e', marginBottom: 6, fontSize: 12 }}>
        {d.class}
      </p>
...
// ── Card wrapper used by stat cards ─────────────────────────────────────────
    ...
    <div className="mt-2">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="pt-5 pb-2 space-y-6 px-0.5">
              ...
              {s && (
                <>
                  ...
                  >
                    ...
                    <Card className="p-4">
                      ...
                      <ResponsiveContainer width="100%" height={200}>
                        <LineChart data={accData} margin={{ top: 8, right: 24, left: -12, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#ddd0bb" />
                          <XAxis dataKey="epoch" tick={{ fontSize: 10, fill: '#6b4c3b', fontFamily: 'DM Sans, sans-serif' }} />
                          <YAxis
                            domain={[75, 100]}
                            tickFormatter={v => `${v}%`}
                            tick={{ fontSize: 10, fill: '#6b4c3b', fontFamily: 'DM Sans, sans-serif' }}
                          />
                          <Tooltip
                            content={<LineTip unit="%" />}
                            {...TIP_STYLE}
                          />
                          <Legend
                            wrapperStyle={{ fontSize: 11, color: '#6b4c3b', fontFamily: 'DM Sans, sans-serif', paddingTop: 8 }}
                          />
                          <ReferenceLine
                            x={`Epoch ${hist.best_epoch}`}
                            stroke="#c9962a"
                            strokeDasharray="5 3"
                            label={{ value: '★ best', fill: '#c9962a', fontSize: 9, position: 'insideTopRight' }}
                          />
                          <Line
                            type="monotone" dataKey="Train" name="Train accuracy"
                            stroke="#2d6a4f" strokeWidth={2.5}
                            dot={{ r: 4, fill: '#2d6a4f', strokeWidth: 0 }}
                            activeDot={{ r: 6 }}
                          />
                          <Line
                            type="monotone" dataKey="Val" name="Val accuracy"
                            stroke="#c4622d" strokeWidth={2.5}
                            dot={{ r: 4, fill: '#c4622d', strokeWidth: 0 }}
                            activeDot={{ r: 6 }}
                          />
                        </LineChart>
                      </ResponsiveContainer>
                    ...
                    {/* Loss */}
                    <Card className="p-4">
                      <SectionLabel>Cross-entropy loss over 3 epochs</SectionLabel>
                      <ResponsiveContainer width="100%" height={200}>
                        <LineChart data={lossData} margin={{ top: 8, right: 24, left: -12, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#ddd0bb" />
                          <XAxis dataKey="epoch" tick={{ fontSize: 10, fill: '#6b4c3b', fontFamily: 'DM Sans, sans-serif' }} />
                          <YAxis
                  ...
                  >
                    {/* Top 10 */}
                    <Card className="p-4">
                      <SectionLabel>Top 10 classes — F1 score</SectionLabel>
                      <ResponsiveContainer width="100%" height={top10.length * 34 + 36}>
                        <BarChart
                          layout="vertical" data={top10}
                        ...
                        >
                          ...
                          <XAxis
                            ...
                            tick={{ fontSize: 9, fill: '#6b4c3b', fontFamily: 'DM Sans, sans-serif' }}
                          />
                          <YAxis
                            type="category" dataKey="label" width={134}
                            tick={{ fontSize: 9.5, fill: '#2c1a0e', fontFamily: 'DM Sans, sans-serif' }}
                          />
                          ...
                          <Bar dataKey="f1" radius={[0, 4, 4, 0]} isAnimationActive>
                            ...
                            <LabelList
                              ...
                              style={{ fontSize: 9, fill: '#6b4c3b', fontFamily: 'DM Sans, sans-serif' }}
                            />
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    ...
                    {/* Bottom 10 */}
                    <Card className="p-4">
                      <SectionLabel>10 Most challenging classes — F1 score</SectionLabel>
                      <p className="text-[10px] mb-3 leading-relaxed" style={{ color: 'var(--color-brown-light)' }}>
                        Visually similar diseases (e.g. Potato Late Blight vs Early Blight) drive most confusion.
                      </p>
                      <ResponsiveContainer width="100%" height={bottom10.length * 34 + 36}>
                        <BarChart
                          layout="vertical" data={bottom10}
                          margin={{ top: 2, right: 36, left: 4, bottom: 2 }}
                        >
                          <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#ddd0bb" />
                          <XAxis
                  ...
                  {/* ── 4. Confusion matrix (collapsible sub-section) ───────── */}
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    ...
                    >
                      <span>Confusion Matrix (38 × 38 classes)</span>
                      ...
                      {showMatrix && (
                        ...
                        >
                          ...
                          <p className="text-[10px] text-center mt-1.5" style={{ color: 'var(--color-brown-light)' }}>
                            38-class confusion matrix · validation set (1,140 images) · best viewed on desktop
                          </p>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  ...
                  <p className="text-[10px] text-center pb-1" style={{ color: 'var(--color-brown-light)' }}>
                    Fine-tuned MobileNetV2 on PlantVillage · {s.train_images.toLocaleString()} training images ·{' '}
                    {s.val_images.toLocaleString()} validation images
                  </p>
                </>
              )}
            </div>
